...
public final class Pipeline: @unchecked Sendable {
  ...
  public init(_ description: String) throws {
    ...
    let parsedPipeline = swift_gst_parse_launch(description, &errorMessage)
    if let errorMessage {
  ...
  ///
  /// - Parameter name: The appsink element name.
  /// - Returns: An ``AudioBufferSink`` wrapper for the element.
  /// - Throws: ``GStreamerError/elementNotFound(_:)`` if not found.
  public func audioBufferSink(named name: String) throws -> AudioBufferSink {
    try AudioBufferSink(pipeline: self, name: name)
  }
  ...
  public struct SeekFlags: OptionSet, Sendable {
    ...
    public static let accurate = SeekFlags(
      rawValue: UInt32(swift_gst_seek_flag_accurate().rawValue))
  ...
  @discardableResult
  public func step(
    amount: UInt64, format: StepFormat = .buffers, rate: Double = 1.0, flush: Bool = true
  ) -> Bool {
  ...
  public struct DebugGraphDetails: OptionSet, Sendable {
    ...
    /// Show all details.
    public static let all = DebugGraphDetails(
      rawValue: UInt32(bitPattern: swift_gst_debug_graph_show_all().rawValue))
